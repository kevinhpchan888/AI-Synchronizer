---
name: book-prod__illustrate
description: Generate book illustrations locally via ComfyUI (FLUX, SDXL) with LumosBooks brand tokens, outputting to assets/source/ for the image-integration pipeline. Trigger when Kevin asks to generate, illustrate, draw, or render book art, cover art, page art, characters, scenes, or coloring-page line art without NanoBanana.
---
...
## Skip when
Kevin explicitly says "use NanoBanana" or "I'll drop the art myself" — in that case do nothing and let `book-prod__image-integration` process whatever he drops.
...
7. After generation, hand off to `book-prod__image-integration` to produce derivatives + sidecar.
...
- Coloring-page line art: 2000×2600 grayscale, then run `book-prod__image-integration` → `gen_coloring.py` to potrace.
