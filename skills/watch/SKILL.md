---
name: watch
description: "Watch a video (URL or local path) like an editor: scene-change frames, pacing metrics, a 0-10s hook microscope, and a transcript from captions or Whisper. Produces report.md and can auto-ingest into the Obsidian vault tied to why you watched. Trigger on watch, analyze, or break down a video."
...
2. **Create the staging dir:** `mkdir -p "$VAULT_DIR/Watched Videos/<slug>"`.
...
4. **Open in Obsidian via URL scheme** (cross-platform). The vault URL-name is the final component of `$VAULT_DIR` with spaces URL-encoded as `%20`. The `file=` path is relative to the vault root with spaces encoded too (`Watched Videos` → `Watched%20Videos`):
   ```bash
   VAULT_NAME=$(basename "$VAULT_DIR" | sed 's/ /%20/g')
   URL="obsidian://open?vault=${VAULT_NAME}&file=Watched%20Videos/<slug>/report.md"
   case "$(uname -s)" in
     Darwin) open "$URL" ;;                          # macOS
     Linux)  xdg-open "$URL" 2>/dev/null || true ;;  # Linux
     *)      cmd.exe /c start "" "$URL" 2>/dev/null || powershell -NoProfile -c "Start-Process '$URL'" ;;  # Windows (Git Bash / MSYS)
   esac
   ```
...
5. **Echo the vault-relative path in chat** on its own line: `📄 Report (open in Obsidian): Watched Videos/<slug>/report.md`. So if Obsidian was closed / the URL handler missed, the user can still navigate to it manually inside the vault.
...
**The vault entry always persists.** Once Step 4.4 has staged `Watched Videos/<slug>/` into the vault, that folder is permanent — every watched video keeps its own entry in the vault no matter what the user answers at Step 4.5. Step 4.5 only decides whether the entry is *also* woven into the wiki (entity/concept pages + `log.md`); it never deletes the staged report or frames. Do not `rm` anything inside `$VAULT_DIR`.

**Step 4.5 — Offer to weave the entry into the rest of the vault.** **Skip this step entirely if no vault was detected at Step 4.4** (the entry has already been saved either way). Otherwise use `AskUserQuestion` once, with these options:

> **Question:** "Saved to your vault under `Watched Videos/<slug>/`. Weave it into your wiki too?"
> - **Yes — same angle** ("<intent>")
> - **Yes — different angle** (user specifies in the notes field)
> - **Just keep the entry** (no wiki changes)
...
2. Confirm the staging dir exists at `$VAULT_DIR/Watched Videos/<slug>/` (Step 4.4 already created it).
...
5. **If `$VAULT_DIR/CLAUDE.md` exists:** Read it to refresh the Ingest op definition — that file is authoritative; this skill must not duplicate its steps. Execute the Ingest op against `Watched Videos/<slug>/report.md` exactly as `$VAULT_DIR/CLAUDE.md` defines it.
6. **If no `$VAULT_DIR/CLAUDE.md` exists:** Run a generic ingest — read the report, identify entities + concepts, append a one-line entry to `$VAULT_DIR/log.md` (create if missing), and tell the user the report is staged at `Watched Videos/<slug>/report.md` and they can wire up an Ingest op of their own.
...
**B. Just keep the entry:**
1. The staging from Step 4.4 already did the file copy — the entry stays.
2. Do NOT touch the wiki. Do NOT append to `log.md`. Do NOT delete anything.
3. Tell the user in chat: "Saved at `$(basename $VAULT_DIR)/Watched Videos/<slug>/`. Say 'ingest the staged report at `<slug>`' anytime to weave it into the wiki."
...
- **Ingest fails partway** → do not roll back. The Second Brain Ingest op is idempotent on re-run (it updates existing pages rather than duplicating). Tell the user what failed, leave the staged artifact in `Watched Videos/<slug>/`, and they can re-run by saying "ingest the staged report at `<slug>`".
...
- Writes a structured `report.md` plus copies of hero frames into `$VAULT_DIR/Watched Videos/<slug>/` when a vault is detected at Step 4.4
