...
- **Optional Obsidian auto-save** — Step 4.4 stages the report into `$VAULT_DIR/Watched Videos/<slug>/` and opens it via the `obsidian://` URL scheme. Step 4.5 offers ingest into the vault's wiki. Both steps skip cleanly when no vault is detected. Vault path is resolved from `$WATCH_VAULT_DIR` or auto-detected from `~/Second brain/`, `~/Documents/Obsidian/`, `~/Obsidian/`.
...
## APC deployment notes (this repo)

This skill is vendored into the APC repo and pre-wired for Kevin's setup:

- **Obsidian auto-save is ON.** `settings.json` sets `env.WATCH_VAULT_DIR = G:/My Drive/Obsidian`, so every watch saves a permanent per-video entry at `G:/My Drive/Obsidian/Watched Videos/<slug>-YYYY-MM-DD/` (report.md + hero frames). The entry always persists; the post-watch prompt only decides whether it's also woven into the wiki. On the Mac Mini the Google Drive mount path differs, so set a per-machine `WATCH_VAULT_DIR` there if you want Hermes agents (Donny) ingesting into the same vault.
- **The Whisper key lives in a local, un-committed file — never in the repo.** Create `~/.config/watch/.env` once per machine and paste your Groq key after the `=`. That file is shared by every watch run on the machine (all PC sessions + every Hermes agent on the Mac Mini), and it is mode `0600`. Get a key at [console.groq.com/keys](https://console.groq.com/keys).

  Windows (PowerShell):
  ```powershell
  New-Item -ItemType Directory -Force "$HOME\.config\watch" | Out-Null
  "GROQ_API_KEY=" | Set-Content "$HOME\.config\watch\.env"   # paste your key after the =
  ```

  macOS / Linux:
  ```bash
  mkdir -p ~/.config/watch
  echo "GROQ_API_KEY=" > ~/.config/watch/.env   # paste your key after the =
  chmod 600 ~/.config/watch/.env
  ```

  Captions cover most public videos, so Whisper only fires on caption-less ones (local files, TikToks, the occasional YouTube upload). Without a key, those come back frames-only.
